        ...
        <div class="max-w-2xl mx-auto space-y-6">
            <div class="bg-white rounded-2xl shadow-sm p-6">
                <form method="POST" action="{{ route('panel.customers.update', $customer) }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500">{{ old('notes', $customer->notes) }}</textarea>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-red-100">
                ...
                <form method="POST" action="{{ route('panel.customers.destroy', $customer) }}" onsubmit="return confirm('هل أنت متأكد من حذف هذا العميل وجميع سجلاته؟');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 rounded-xl bg-red-600 text-white text-sm font-medium hover:bg-red-700">حذف العميل نهائيًا</button>
                </form>
            </div>
        </div>
    </div>
